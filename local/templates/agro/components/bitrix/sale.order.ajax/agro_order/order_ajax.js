    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        getDeliveryLocationInput: function (node) {
            ...
            if (location && location[0] && location[0].output) {
                ...
                labelHtml = '<label class="name">' +  BX.util.htmlspecialchars(currentProperty.NAME) + '</label>';

                currentLocation = location[0].output;

                insertedLoc = BX.create('DIV', {
                    attrs: {'data-property-id-row': locationId},
                    props: {className: 'checkout__info-step__address form-group'},
                    style: {visibility: 'hidden'},
                    html: labelHtml + currentLocation.HTML
                });

                insertedLoc.appendChild(
                    BX.create('p', {
        ...
        getZipLocationInput: function (node) {
            ...
            if (zipProperty) {
                ...
                propsItemNode = BX.create('DIV', {props: {className: "checkout__info-step__address form-group"}});
                ...
                label = BX.create('DIV', {
                    ...
                    text: BX.util.htmlspecialchars(zipProperty.NAME)
                });
                input = BX.create('INPUT', {
                    props: {
                        id: 'zipProperty',
        ...
        getPersonTypeControl: function (node) {
            ...
            if (personTypesCount > 1) {
                input = BX.create('DIV', {
                    props: {className: 'checkout__info-step__type form-group'},
                    children: [
                        BX.create('DIV', {props: {className: 'name'}, html: this.params.MESS_PERSON_TYPE}),
                    ]
                });
        ...
        createPaySystemItem: function (item) {
            var checked = item.CHECKED == 'Y',
                logotype, logoNode, logoImg,
                paySystemId = parseInt(item.ID),
                title, label, input, itemNode;

            logoNode = BX.create('DIV', {props: {className: 'ico'}});
            logotype = this.getImageSources(item, 'PSA_LOGOTIP');
            if (logotype && logotype.src_2x) {
                logoImg = BX.create('IMG', {props: {src: logotype.src_1x}});
            } else {
                logotype = logotype && logotype.src_1x || this.defaultPaySystemLogo;
                logoImg = BX.create('IMG', {props: {src: logotype}});
            }
        ...
        editDeliveryItems: function (deliveryNode) {
            ...
            if (this.deliveryPagination.show)
                this.showPagination('delivery', deliveryNode);
        ...
        editDeliveryInfo: function (deliveryNode) {
            ...
            deliveryInfoContainer.appendChild(
                BX.create('DIV', {
                    props: {className: 'bx-soa-pp-company'},
                    children: [subTitle, logoNode, title, clear, extraServicesNode, infoList]
                })
            );
        ...
        createDeliveryItem: function (item) {
            var checked = item.CHECKED == 'Y',
                ...
                logotype, label, title, itemNode, logoNode, logoImg;
