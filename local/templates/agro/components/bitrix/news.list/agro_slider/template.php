        ...
        <?php foreach ($arResult['ITEMS'] as $arItem): ?>
            ...
            <div class="main-top" id="<?= $this->GetEditAreaId($arItem['ID']); ?>">
                <div class="container clearfix lazy"
                    ...
                    <a class="main-top__link" href="<?= $arItem['PROPERTIES']['LINK']['VALUE'] ?>"></a>
                    <?php if (!empty($arItem['PREVIEW_TEXT'])): ?>
                        <div class="zag--black"><?= $arItem['PREVIEW_TEXT']; ?></div>
                    <?php endif; ?>
                    <?php if (!empty($arItem['PROPERTIES']['OUR_PHONES']['VALUE'])): ?>
                        <div class="phone-block">
                            ...
                            <ul class="phone-block__ul">
                                <?php foreach ($arItem['PROPERTIES']['OUR_PHONES']['VALUE'] as $item): ?>
                                    <li><?= htmlspecialchars_decode($item['TEXT']) ?></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                    <?php endif; ?>
                    <?php if (!empty($arItem['PROPERTIES']['ADRESS']['VALUE'])): ?>
                        <div class="location-block">
                            <ul class="location-block__ul">
                                <?php foreach ($arItem['PROPERTIES']['ADRESS']['VALUE'] as $key => $item): ?>
                                    <li class="ico-placeholder">
                                        <a href="/contacts/">Магазин на<span><?= $item ?></span></a>
                                        <?php if (!empty($arItem['PROPERTIES']['PHONE']['VALUE_ENUM'][$key])): ?>
                                            <a href="tel:<?= $arItem['PROPERTIES']['PHONE']['VALUE_ENUM'][$key] ?>">
                                                <?= $arItem['PROPERTIES']['PHONE']['VALUE_ENUM'][$key] ?>
                                            </a>
                                        <?php endif; ?>
                                    </li>
                                <?php endforeach; ?>
